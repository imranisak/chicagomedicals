...
if(isset($_POST['surname'])) {
    $newsurname = filter_var($_POST['surname'], FILTER_SANITIZE_STRING);
...
if(!$userClinics){
    $databaseConnection->close();
    $msg->error("Profile updated, but an error happened while loading your clinics.", $redirectLink);
}
...
foreach ($userClinics as $clinic){
    ...
    if(!$update) {
        $databaseConnection->close();
        $msg->error("Error updating clinics info.", $redirectLink);
    }
}
$databaseConnection->close();
